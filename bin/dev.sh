...
print_status() {
    if is_running; then
        local pid
        pid="$(cat "$PID_FILE")"
        echo "✅ PennyTrack dev server is RUNNING"
        echo "   PID:     $pid"
        ...
        echo "⛔ PennyTrack dev server is STOPPED"
    fi
}
...
start() {
    ...
    echo "→ Starting PennyTrack dev server on ${HOST}:${PORT}…"
